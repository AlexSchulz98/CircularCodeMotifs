...
Change here:
```{r}
pathSeq = "D:/Projekte/BA Circular Code/output_sequences/Human/"
filenames = list.files(pathSeq, pattern = "*.fasta")

pathMatrix = "D:/Projekte/BA Circular Code/output_matrixes_codons/Human/"
matrixnames = list.files(pathMatrix, pattern = "*.RDS")
```

Iterate through all files. Result is stored in data frame:
```{r echo=FALSE}
df = data.frame(Code=c(),
                Frame=c(),
                Code_Usage=c(),
                Changed_Codons=c(),
                Sample_MML=c(),
...
names(df) = c("Code","Frame","Achieved Code Usage","% of Codons changed","Sample Mean Motif Lenght","Expected Mean Motif Lenght","Edit Score","Edit Distance")
