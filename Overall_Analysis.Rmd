...
names(df) = c("Organism","Code","Stop-Codons in Code?","Frame","Achieved Code Usage","% of Codons changed","Sample Mean Motif Length","Expected Mean Motif Length","Diff. Sample & Exp. Motif Length","Max. Motif Length","Edit-Score","Edit-Distance")
...
Splitted by Organism
```{r}
# df_Org = split(df, df$Organism)
# 
...
#     # saveRDS(
#     # object = df_Org[i],
#     # file = paste("analysis_results/",name,"_Analysis.RDS",sep=""))
# 
#     write.xlsx(df_Org[i], file = "analysis_results/Summary_Analysis.xlsx",
#       sheetName = org, append = TRUE)
# }

```

Split by organism and stop-codons in code:
```{r}
# df_Org_Stop = split(df, list(df$Organism, df$`Stop-Codons in Code?`))
# 
# for (i in 1:length(df_Org_Stop)) {
#   org_stop = names(df_Org_Stop[i])
#   
#     # saveRDS(
#     # object = df_Org[i],
#     # file = paste("analysis_results/",name,"_Analysis.RDS",sep=""))
# 
#     write.xlsx(df_Org_Stop[i], file = "analysis_results/Summary_Analysis.xlsx",
#       sheetName = org_stop, append = TRUE)
# }
...
```{r}
df_cu = df[order(-df$`Achieved Code Usage`),]

write.xlsx(df_cu, file = "analysis_results/Summary_Analysis.xlsx",
      sheetName = "Sorted by Code Usage", append = TRUE)
```

Sorted by % codons changed
```{r}
df_change = df[order(-df$`% of Codons changed`),]

write.xlsx(df_change, file = "analysis_results/Summary_Analysis.xlsx",
      sheetName = "Sorted by % changed", append = TRUE)
```

Sorted by difference between sample and expected motif length
```{r}
df_diff = df[order(-df$`Diff. Sample & Exp. Motif Length`),]

write.xlsx(df_diff, file = "analysis_results/Summary_Analysis.xlsx",
      sheetName = "Sorted by Diff", append = TRUE)
```
